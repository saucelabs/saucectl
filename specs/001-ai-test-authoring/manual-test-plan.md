...
**Expected**
- `Error: invalid region "mars"; options: us-west-1, us-east-4, eu-central-1, asia-south-2`; exit 1.
- The list is derived from the region package, not hardcoded, so it always matches the regions the
  binary actually accepts. `staging` is accepted but deliberately not advertised.
...
- Step 2: `Error: --timezone "UTC" is not accepted by the service; use an IANA region/city zone, e.g.
  Atlantic/Reykjavik for a zero offset`; exit 1, and **no request is sent**. The guard is client-side
  (`internal/cmd/authoring/schedules_create.go`), so the service's own `INVALID_BODY ... settings.timezone`
  error is no longer reachable through this command.
...
- Variant: press Ctrl-C **before** `Run started` appears (during the entitlement check). **Known defect**
  (found 2026-09-16): the documented `WRN Run was not started: interrupted.` does **not** appear. The
  observed output is a raw transport error that exposes the internal URL and never says the run was
  interrupted:
  `ERR failed to execute run command error="could not verify AI authoring entitlement: resolving the current user failed: Get \"https://api.<region>.saucelabs.com/team-management/v1/users/me\": context canceled"`
  Cause: the message lives in `Runner.logResult` (`internal/authoring/runner.go`) behind
  `res.Interrupted && res.Run.ID == ""`, which is only reached once the runner is processing cases. An
  interrupt during the entitlement gate happens before the runner exists. Severity **Major** (misleading
  message), not a blocker. Record as seen until fixed.
...
- Step 2: **`1`, not `0`** — and this is pre-existing on `main`, not caused by this PR. `.sauce/cypress-10.yml`
  pins `cypress.version: 13.15.2`, which has been dropped from the schema enum (oldest 13.x is now
  `13.16.0`), so the advisory validator reports
  `value must be one of "package.json", "15.18.0", … "13.16.0" in /cypress/version`.
  Confirm it is not a regression with `git diff origin/main -- .sauce/cypress-10.yml api/v1/framework/cypress.schema.json`
  (empty means both files match `main`). The scenario passes as long as that count is unchanged between
  the branch and `main`; fixing it means bumping the sample config, which belongs to `main`.

### 12.3 Unrelated groups unaffected

**Steps**
1. `./saucectl storage list | head -3`
