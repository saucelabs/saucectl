...
exit codes, use 'saucectl run' with a 'kind: authoring' configuration instead.`,
  ...
  saucectl authoring variables create --scope org --name password --secret --value-from-env PASSWORD`,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if reg == region.None {
				return fmt.Errorf("invalid region %q; options: %s", regionFlag, region.Options())
			}
	...
	cmd.PersistentFlags().StringVarP(&regionFlag, "region", "r", "us-west-1", fmt.Sprintf("The Sauce Labs region. Options: %s.", region.Options()))
