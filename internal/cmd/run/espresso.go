...
func NewEspressoCmd(viper *viper.Viper) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			exitCode, err := runEspresso(cmd, viper, lflags, tcClient, restoClient, rdcClient, appsClient)
			if err != nil {
	...
	sc := flags.SnakeCharmer{Viper: viper, Fset: cmd.Flags()}
...
func runEspresso(cmd *cobra.Command, viper *viper.Viper, flags espressoFlags, tc testcomposer.Client, rs resto.Client, rc rdc.Client,
	as appstore.AppStore) (int, error) {
	p, err := espresso.FromFile(viper, gFlags.cfgFilePath)
	if err != nil {
		return 1, err
	}
